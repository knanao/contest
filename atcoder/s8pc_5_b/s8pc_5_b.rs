...
fn main() {
    ...
    let mut xyr = vec![(0f64, 0f64, 0f64); n];
    for i in 0..n {
        xyr[i] = (sc.next(), sc.next(), sc.next());
    }

    let mut xy = vec![(0f64, 0f64); m];
    for i in 0..m {
    ...
    for i in 0..n {
        let (x, y, r) = xyr[i];
        for j in 0..m {
            let d = ((x - xy[j].0).powf(2.0) + (y - xy[j].1).powf(2.0)).powf(0.5);
            v.push(d - r as f64);
        }
    }
    ...
    for i in 0..m {
        for j in (0..m).filter(|&j| j != i) {
            let d = ((xy[i].0 - xy[j].0).powf(2.0) + (xy[i].1 - xy[j].1).powf(2.0)).powf(0.5);
            v.push(d / 2.0);
        }
    }
