...
fn main() {
    ...
    loop {
        let (n, w, d): (usize, usize, usize) = (sc.next(), sc.next(), sc.next());
        if n == 0 && w == 0 && d == 0 {
            return;
        }
        let mut ps: Vec<(usize, usize)> = vec![];
        for _ in 0..n {
            ps.push((sc.next::<usize>() - 1, sc.next()));
        }

        let mut cakes = vec![(w, d)];
        for (p, mut s) in ps {
            let (wi, di) = cakes[p];
            s %= wi + di;
            cakes.remove(p);
            ...
            let large: (usize, usize);
            if s < wi {
                small = (std::cmp::min(wi - s, s), di);
                large = (std::cmp::max(wi - s, s), di);
            } else {
                small = (wi, std::cmp::min(di + wi - s, s - wi));
                large = (wi, std::cmp::max(di + wi - s, s - wi));
            }

            cakes.push(small);
            cakes.push(large);
        }
        cakes.sort_by_key(|&(w, d)| w * d);
        for (i, &(w, d)) in cakes.iter().enumerate() {
            if i == cakes.len() - 1 {
                println!("{}", w * d);
                return;
            }
